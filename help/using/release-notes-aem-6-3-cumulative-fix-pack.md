...
| **Requisitos previos** | [AEM Paquete de servicio 3 de.3 (6.3.3.0)](https://helpx.adobe.com/es/experience-manager/6-3/release-notes/sp3-release-notes.html) |
...
El paquete de correcciones acumulativas 6.3.3.8 de AEM depende del paquete de servicio 3 de AEM 6.3. Por lo tanto, debe instalar el paquete de correcciones acumulativas 6.3.3.x de AEM después de instalar el paquete de servicio 3 de AEM 6.3. Para obtener instrucciones de instalación, consulte [las notas de la versión del paquete de servicio 3 de AEM 6.3](https://helpx.adobe.com/es/experience-manager/6-3/release-notes/sp3-release-notes.html).
...
>[!NOTE]
>
>AEM MongoDB Enterprise 3.6 es compatible a partir de la versión 6.3.3.1 de la.
...
* Las propiedades de página de una página principal no se pueden cargar y se devuelve NullPointerException. El problema se resuelve al agregar la propiedad cq:blueprint (NPR-30901).
* Las configuraciones de despliegue no se recuperan correctamente de blueprintConfig en el nodo raíz. La desactivación se activa tanto para modelos como para Live Copies. La desactivación solo debe activarse para el modelo (NPR-30866).
...
* No se puede sincronizar la campaña en la campaña de Adobe que contiene el componente de importador de diseño predeterminado (NPR-30890).
* Editor de texto enriquecido (RTE). no permite insertar un objeto Table incrustado como elemento de lista (NPR-30878).
...
* AEM Los informes de Analytics no se rellenan en entorno de creación, por lo que aparece una página en blanco (NPR-30905).
...
* La agregación de vídeo por vídeo dinámico devuelve solo los 100 elementos principales del conjunto de resultados. NPR-30441; revisión para CQ-4213561
* Adobe de problemas de conectividad de etiquetas inteligentes a través de Datapower. NPR-30026: revisión para CQ-4269457
...
* AEM Error de ContextHub durante la integración de los servicios de asistencia y Campaign. NPR-30624: revisión para CQ-4250790
* AEM Las propiedades de metadatos &quot;onTime&quot; u &quot;offTime&quot; guardadas en los recursos no se recuperan si se reinicia el servidor de. NPR-30412: revisión para CQ-4272784
...
* Al utilizar el marco de trabajo de traducción, la adición de páginas a varios trabajos de traducción déclencheur un error. NPR-29746: revisión para CQ-4270953
...
* Cuando se realiza una prueba de carga en el servicio HTML a PDF AEM, se produce un error y la configuración del tipo de archivo se elimina del servidor de formularios de la forma en que se ha realizado la prueba de carga en el servidor de formularios de la manera siguiente: NPR-30111, NPR-30086: revisión para CQ-4271495
...
El paquete de correcciones acumulativas 6.3.3.5 de AEM depende del paquete de servicio 3 de AEM 6.3. Por lo tanto, debe instalar el paquete de correcciones acumulativas 6.3.3.x de AEM después de instalar el paquete de servicio 3 de AEM 6.3. Para obtener instrucciones de instalación, consulte [las notas de la versión del paquete de servicio 3 de AEM 6.3](https://helpx.adobe.com/es/experience-manager/6-3/release-notes/sp3-release-notes.html).
...
* Los valores son diferentes entre la página de colección y la página de colección mediante el filtro. NPR-28642: revisión para CQ-4261405
* Déclencheur CommitFailedException con carga de recursos de archivo zip grandes. NPR-28528: revisión para CQ-4260903
* Los metadatos de carpeta no se pueden guardar al editar una carpeta que contiene caracteres especiales. NPR-28211: revisión para CQ-4260401
* No se pueden eliminar las representaciones de imagen de un recurso de vídeo de la página Detalles del recurso. NPR-29149: revisión para CQ-4266073
* La entrega de vídeo de escritorio de DMS7 mediante DMComponent utiliza la descarga progresiva para reproducir vídeo en modo de publicación y no transmite. NPR-28754: revisión para CQ-4263732
* No se pueden crear o editar ajustes preestablecidos de imagen porque la restricción del acceso a /etc/dam/video/dynamicmedia desactiva las funciones de los administradores de imágenes. NPR-28662: revisión para CQ-4263022
* Los vínculos compartidos con archivos de vídeo codificados DMS7 generan carpetas vacías. NPR-28851: revisión para CQ-4206743
* AEM La replicación de la a Brand Portal se queda atascada durante largos períodos. NPR-28913: revisión para CQ-4254932

### Communities {#communities-3}

* No se pueden abrir los mensajes con datos adjuntos en las carpetas de la bandeja de entrada y enviados de Outlook. NPR-28559: revisión para CQ-4217072

### Sites {#sites-4}

* Ejecución de scripts en sitios múltiples (XSS) en SuggestionHandler para 6.3. NPR-28692: Revisión para CQ-4253821
* Finaliza el despliegue profundo sin contener todas las ramas de la LiveCopy correspondiente. NPR-29175: revisión para CQ-4239472
...
* Abrir la variación en la vista de tarjeta provoca un error 500. NPR-28611: revisión para CQ-4263571
...
* El uso de las credenciales de Brightedge produce un error de conexión. NPR-29167: revisión para CQ-4265872
* Problema de compilación en offerProxyTandtProvider.java debido a la falta de la instrucción de importación para la clase &quot;Resource&quot;: falta la instrucción de importación: import org.apache.sling.api.resource.Resource. NPR-28772
...
* Se ha agregado la funcionalidad Compartir cola para todos los exploradores.
...
* No se puede enviar desde el espacio de trabajo si el botón contiene el carácter Umlaut. NPR-29014: revisión para CQ-4263172
...
* El repositorio integrado (Apache Jackrabbit Oak) se ha actualizado a la versión 1.6.16.
* La paginación de la lista de configuración de directivas cambia para limitar los 50 registros por página.
...
* Se ha incluido un carácter de escape para la coma cuando se realiza cualquier búsqueda.
* Se habilitó la compatibilidad de recursos sintéticos para la directiva de contenido.
...
* Se ha habilitado la navegación mediante el teclado para los componentes de habilitación de AEM Communities en Publish. NPR-27739: revisión para CQ-4253856
* Se habilitó la navegación mediante el teclado para reproducir contenido de déclencheur. NPR-27738: revisión para CQ-4254026
...
* Los comentarios publicados en la página siguiente aparecen resaltados en amarillo cuando se publica un comentario nuevo. NPR-28148: revisión para CQ-4259681
...
* El resaltado de anotación desaparece en cuanto el usuario hace clic en la anotación. BPR-27196: revisión para CQ-4254423
...
* Mejore la consulta para utilizar la función Notificación de caducidad. NPR-26999: revisión para CQ-4251188
* Migración de reglas del esquema de metadatos al esquema de metadatos de carpeta. NPR-27771: Backport para CQ-4257737, CQ-4257735, CQ-4259822
...
* La herencia de la configuración del Cloud Service se interrumpe al editar otras propiedades de página. NPR-27311: revisión para CQ-4256785
...
* Cuando el editor de texto enriquecido se combina con varios campos, se produce el error TypeError: fieldAPI.getName no capturado no es una función en foundation.js. NPR-27146: revisión para CQ-4253155, CQ-4259967
...
* Error al cargar un archivo adjunto que no es una imagen (por ejemplo, .pdf) en el foro. NPR-27375: revisión para CQ-4257753
* El vínculo para las respuestas no funciona con los clics en filas. NPR-24556: revisión para CQ-4256138
...
* La etiqueta de estado de finalización del recurso de SCORM de habilitación aparece vacía en la IU. NPR-27152: revisión para CQ-4249994
* Al habilitar &quot;Permitir privilegiados&quot;, los miembros privilegiados solo deben poder componer para usuarios que sean miembros de la comunidad. NPR-27901: revisión para CQ-4251235
...
* Problema de concurrencia con el SDK de cliente JEE. NPR-27572: revisión para CQ-4247156
...
* El componente Contenedor de AEM Forms se vuelve invisible una vez que se configura el formulario incorrecto en la página de Sites y se selecciona la casilla de verificación &quot;Los formularios cubren todo el ancho de la página&quot;. NPR-25972: revisión para CQ-4239287, CQ-4249133
...
* Se ha corregido un problema en el control FormData para el valor predeterminado.
* Se ha actualizado FormBuilder a la última versión de Handlebars.
* Se ha agregado un servlet de configuración para la configuración de edición para RTE en el modo de diálogo.
...
* No se abre la descompresión de un archivo creado por el paso sin archivador con una carpeta cuyo nombre tiene un %. NPR-26275: revisión para CQ-4251482
...
* El uso de GQLConverter con una búsqueda de texto completo que contenga las letras &quot;O&quot; dentro del texto se procesa incorrectamente. NPR-26564: revisión para CQ-4247362
* Al compartir un vínculo de recurso de varios inquilinos, se antepone el ID del inquilino, lo que crea una URL no válida. NPR-26482: revisión para CQ-4253540
...
* La ingesta de archivos .mos RAW está prolongada. NPR-26296: revisión para CQ-4250661
...
* Problema de utilización de memoria de montón con generación de miniaturas. NPR-25545: revisión para CQ-4246960
* La creación de un gran número de relaciones en un recurso provoca un error. NPR-26309: revisión para CQ-4250708
...
* Correcciones proactivas de Backport of Sites. NPR-26963
...
* Preguntas sobre el almacenamiento en caché de Client Context segment.js y no almacenamiento en caché. NPR-26622: revisión para CQ-4253486
...
* AEM El TSDK incrustado con la se rompe con una actualización a Handlebars 4 debido al uso de plantillas incompatibles. NPR-26699: revisión para CQ-4248974
...
* El transporte HTTP puede filtrar las sesiones abiertas. Revisión para Granite-17434
* Excepciones en los registros mientras varios agentes de replicación acceden simultáneamente al nodo de token de acceso. NPR-26964: Revisión para GRANITE-23276, Granite-23155
...
* Al eliminar una copia de un recurso de imagen, el recurso de imagen original no se puede utilizar. Revisión para CQ-4251648
* Descarga redundante de contenido de imagen adicional desde servidores S7. Revisión para CQ-4248770

#### Granite {#granite}

* AEM El proveedor de OAuth no realiza la búsqueda sin distinción de mayúsculas y minúsculas. NPR-26133: revisión para GRANITE-22650
...
* La notificación &quot;Marcar todo leído&quot; solo procesa los 10 primeros como no leídos sin actualizar la página. NPR-27036: revisión para CQ-4254058
...
* La notificación no leída no aumenta más de 21. NPR-26947: revisión para CQ-4251251
...
* La ruta de Tomcat Context(/aempublish) se borra al desplazarse por la página Asignaciones. NPR-26919: revisión para CQ-4254345
* NullPointerException al crear un grupo y un miembro. NPR-26778: revisión para CQ-4248095
* El moderador que abre y el contenido no destacado no funciona con el Principio de responsabilidad única (SRP) de MySQL. NPR-26767: revisión para CQ-4251520
* Problemas de funcionalidad de búsqueda con ciertos caracteres. NPR-26726: revisión para CQ-4247744
* Habilite vínculos profundos para los recursos de habilitación e IU de moderación. NPR-26705: revisión para CQ-4251381
* Problema de búsqueda en el componente del foro. NPR-26577: revisión para CQ-4251303
* La búsqueda conjunta del nombre y los apellidos en la mensajería de comunidades no arroja los resultados esperados. NPR-26377: revisión para CQ-4253150
* La paginación no se actualiza al eliminar las respuestas. NPR-26327
* La eliminación de la publicación funciona, pero genera un error en la consola y la publicación permanece visible en la interfaz de usuario. NPR-26292: revisión para CQ-4252803
* La paginación no se actualiza dinámicamente y la lista de respuestas sigue aumentando. NPR-26145: revisión para CQ-4251586
* La configuración de grupo no se carga en un grupo que contiene 50 000 ~ 100 000 usuarios. NPR-25642: revisión para CQ-4238426
* Error del reproductor de recursos de catálogo en las rutas de contexto. NPR-25640: revisión para CQ-4238426
...
* Los grupos no son visibles al crear un sitio. NPR-27148: revisión para CQ-4251788
...
* La edición del sitio y la adición de miembros en la configuración de funciones producen una excepción de puntero nulo. NPR-27132: revisión para CQ-4255909

#### Interfaz de usuario {#user-interface}

* Correcciones proactivas de inicio de sesión en Platform. NPR-26961
...
* Correcciones proactivas de CQ/registro de experiencias. NPR-26943
* Backports de IU proactivos de Foundation. NPR-26942
...
* Excepciones de sustitución de clave principal en los registros del servidor para claves inactivas. NPR-26748: revisión para CQ-4253705
* No se puede crear o modificar la configuración de marca de agua de Document Security. NPR-26267, NPR-26129: revisión para CQ-4250234
...
* El PDF y la Vista previa del HTML de la carta son visibles y funcionales en la misma ventana cuando se edita, guarda y, a continuación, se abre o cierra la Vista previa del PDF. NPR-26770: revisión para CQ-4253217

#### Forms: flujo de trabajo {#forms-workflow-2}

* Workspace se bloquea intermitentemente y muestra los puntos de inicio repetidamente. NPR-26461: revisión para CQ-4253439
* Se produce una excepción ESAPI en los registros cuando se utilizan llaves en el nombre de la tarea. Revisión para CQ-4256627
* Se produce una excepción de puntero nulo cuando se abre un punto de inicio asociado de un formulario o conjunto de formularios adaptable no rellenado previamente. Revisión para CQ-4256124
* No se puede enviar correo electrónico con la configuración global. NPR-26163: revisión para CQ-111110
...
* Guardar cambios en la pantalla de preferencias de HTML Workspace genera una excepción. Revisión para CQ-4241485
* La aprobación masiva se ha dañado en la última configuración de JEE. Revisión para CQ-4241486
* El borrador de la tarea o el punto de inicio produce errores en el último servidor JEE de 6.4. Revisión para CQ-4241484
...
* Revisión de código para la API de seguridad empresarial. Revisión para CQ-4255638
* Error al cargar esapi.properties como recurso de cargador de clases en WAS9. Revisión para CQ-4255631
...
* Error de org.jgroups.Message en los registros después de actualizar de LiveCycle AEM a formularios de la versión 6.2 de la. NPR-26795: revisión para CQ-4220415
* AEM Formularios de: error al migrar estilos. Revisión para CQ-4251969
* Se han corregido los problemas notificados en el informe de análisis de código estático de PDFG. NPR-23251: revisión para CQ-4213930
...
* Se habilitaron subrecursos y visualizador de recursos de varias páginas en Brand Portal.
...
* Se ha configurado un mensaje de error del servidor cuando una imagen infringe los criterios de carga.
* Se ha añadido la compatibilidad con STARTTLS en el servicio de correo de Day CQ.
* Actualice a las últimas versiones de cq-wcm-content y com.adobe.cq.launches.it.serverside.
* Actualice com.adobe.granite.ui.coralui3-rte a la versión más reciente lanzada.
...
* El campo Grupo de usuarios cerrados (CUG) de la carpeta de recursos no devuelve el grupo &quot;todos&quot;. NPR-23163: revisión para CQ-4239377
* Buscar búsquedas guardadas de colecciones inteligentes no devuelve todos los resultados. NPR-23243: revisión para CQ-4240355
...
* La falta de coincidencia entre el tamaño de la descarga y la información mostrada es incorrecta, lo que provoca confusión en los usuarios. NPR-23418: revisión para CQ-4242774
...
* Las facetas de búsqueda de administración de recursos desaparecen en los paneles de búsqueda después de la previsualización. NPR-23964: revisión para CQ-4244410
* Cancelar la publicación del formulario de búsqueda quita completamente el formulario de búsqueda predeterminado. NPR-23291: revisión para CQ-4241382
...
* La acción Publicar en la IU de Brand Portal no está disponible. NPR-23293: revisión para CQ-4241161
* El botón Publicar/Cancelar la publicación en Brand Portal no debe estar disponible para fragmentos de contenido. NPR-23318: revisión para CQ-4245086
...
* Problemas de visualización/selección de representaciones dinámicas con un recuento preestablecido de imagen grande. NPR-23151: revisión para CQ-4217511
* Problema con el vídeo de codificación de Dynamic Media: modificación/recarga del lanzador. NPR-23237: revisión para CQ-4240260
...
* Parsys rompe la anchura del marco del dispositivo emulador y recorta cualquier componente añadido a él. NPR-22926: revisión para CQ-4238224
...
* El editor de plantillas solo muestra 20 plantillas/carpetas y no carga las demás al desplazarse hasta la parte inferior de la página. NPR-23185: revisión para CQ-4238483
* (IU clásica) Se genera un error al mover o cambiar el nombre de las páginas. NPR-23213: revisión para CQ-4240971
* No se pueden editar o crear segmentos de ContextHub. NPR-23218: revisión para CQ-4226948
...
* Falta el botón Despliegue en la pestaña Modelo para las variaciones de XF. NPR-23320: revisión para CQ-4240404
* La IU clásica no funciona para editar CUG ya que ha quedado obsoleta. NPR-24122: Revisión para 4241823
* Corrección proactiva para protegerse contra promociones de contenido no deseadas. NPR-24387: Revisión para 4244993
* Una vez añadidos aproximadamente Se han agregado 80 fragmentos en una carpeta de Assets. Se detectan errores cuando el flujo de trabajo se activa desde la consola Cronología. NPR-23393; revisión para CQ-4211216
...
* Las opciones del sistema de estilos aparecen en todas las páginas posteriores a la instalación de la versión 6.3.2.1. NPR-23972: revisión para CQ-4244394
* La IU clásica no funciona para editar CUG ya que ha quedado obsoleta. NPR-24122: Revisión para 4241823
* Corrección proactiva para protegerse contra promociones de contenido no deseadas. NPR-24387: Revisión para 4244993
* Las referencias a recursos no se actualizan cuando se mueven. NPR-23208: revisión para CQ-4239879

### Plataforma {#platform-1}

* La colección inteligente no muestra los resultados después de guardar si se utiliza el predicado de etiquetas en la faceta de búsqueda. NPR-23401: Revisión para Granite-21278
...
* El nombre de la etiqueta de queja JCR debe rellenarse automáticamente en función del Título de la etiqueta. NPR-24173: revisión para CQ-4199411
...
* La replicación de referencia de configuración falla con una estructura de carpetas intermedia. NPR-23485: revisión para CQ-4242751
...
* La tabla Foundation es vulnerable a la ejecución almacenada de scripts en sitios múltiples. NPR-23214: revisión para CQ-4240760
...
* Coral.ColumnView: Agregar compatibilidad con Mayús+clic. NPR-23404: Revisión para Granite-13338
* La opción Seleccionar RT no se valida cuando se selecciona un elemento con un valor vacío. NPR-23405: Revisión para Granite-21283
* (OMEGA) Informe “Función” solo en inglés. NPR-23990: Revisión para Granite-21231
* Correcciones en la API Coral.Autocomplete. NPR-23516

### Granite {#granite-1}

* AEM Las conexiones de seguimiento del cliente Http de Apache y la alta utilización de la pila hacen que el servidor se bloquee. NPR-23906: Revisión para Granite-21056
...
* La búsqueda en las comunidades está fallando por unas pocas palabras. NPR-23256: revisión para CQ-4240717
* No se pudieron asignar grupos para el problema de rol de administradores de la comunidad. NPR-23317: revisión para CQ-4241233: CQ-4221399
* Problemas en la creación de recursos con un nombre de recursos similar. NPR-23319: revisión para CQ-4240700
...
* AEM imsmanifest.xml no se puede cargar con la ejecución de un archivo con la raíz de contexto. NPR-23483: revisión para CQ-4242193
...
* Los problemas con los componentes de las comunidades cuando se accede a ellos a través de Dispatcher y sling dynamic incluyen habilitado para ello. NPR-23586: revisión para CQ-4242360, CQ-4241522
* Al buscar un término de búsqueda que produce muchos resultados mediante la búsqueda y, a continuación, al introducir un nuevo término de búsqueda, la paginación no se restablece. NPR-23739: revisión para CQ-4222593
...
* No se muestran detalles sobre los usuarios de indicador y los motivos mediante la configuración de DSRP. NPR-23973: revisión para CQ-4243205
* Los motivos de marca de los usuarios que no están marcados permanecen visibles/ NPR-23974: Revisión para CQ-4243822
* Al adjuntar dos archivos con el mismo nombre dos veces a una publicación de formulario, se produce un error del servidor. NPR-24166: revisión para CQ-4244367
* No se pueden almacenar datos adjuntos con tipos MIME de más de 15 caracteres mediante el Desarrollador de recursos de almacenamiento de bases de datos (DSRP). NPR-24174
* Cuando se arrastra una imagen que infringe los criterios de carga y se suelta en el texto del anuncio, se produce un error de servidor y se muestra un mensaje de error genérico al usuario. NPR-24243
* Correcciones de varios problemas del lado del servidor de AEM Communities. NPR-23971: revisión para CQ-4243144, CQ-4242145, CQ-4243365, CQ-4244098
...
* Para incluir SDLInvokerParams en el SDK de cliente de complementos de Forms. NPR-23157
...
* El servicio Paper Capture se bloquea mientras se procesan simultáneamente los archivos del TIFF ( tamaño de unos 50 KB ). NPR-23556
...
* Se ha mejorado la representación de los componentes de Parsys en la IU clásica.
* Se han actualizado los paquetes de modelos Sling para incluir la corrección del conjunto de caracteres.
* La publicación en Brand Portal se ha realizado de forma asíncrona para todos los tipos de recursos.
...
* Se ha habilitado el volteado de imágenes para el componente de imagen principal.
* Se han actualizado los paquetes http felix para habilitar los atributos de sesión.
...
* La IU genera el error falso &quot;Error de publicación en Brand Portal&quot;, mientras que el trabajo se agrega a la cola de replicación y los recursos se publican en Brand Portal. NPR-22179: revisión para CQ-4205273
...
* Error al intentar descomprimir un archivo ZIP debido al espacio final al final del nombre de la carpeta. NPR-22522: revisión para CQ-4238036
* La ordenación mediante la columna Título del recurso no funciona para los resultados de búsqueda. NPR-22908: revisión para CQ-4239076
* El vídeo de YouTube se etiqueta con la ruta completa en lugar del propio nombre de la etiqueta. NPR-22976: revisión para CQ-4238669
* La reordenación de carpetas en una carpeta reordenable no persiste. NPR-23125: revisión para CQ-4231761
* HTTP 504: error de tiempo de espera de puerta de enlace al intentar compartir colecciones mediante un vínculo compartido. NPR-21928: revisión para CQ-4234507
* Los metadatos de palabra clave de PDF no se extraen correctamente y se modifican incorrectamente cuando hay varias palabras clave asociadas a un recurso de PDF. Para resolver el problema, se ha eliminado la propiedad de metadatos del campo Asunto para los recursos del PDF. Sin embargo, puede editar el esquema de metadatos para agregar un campo de texto de varios valores para el campo Asunto. NPR-21972: Revisión para 4215741****
* Los recursos incorrectos se eliminan si la carpeta seleccionada se cambia entre el momento en que se muestran las dos ventanas emergentes. NPR-21980: revisión para CQ-4233675
...
* Hacer que el uso compartido público sea la opción predeterminada al publicar recursos en Brand Portal. NPR-21931: revisión para CQ-4218816
...
* Los componentes de estructura editables pierden los nombres de clase CSS necesarios para la cuadrícula adaptable cuando se editan. NPR-21741: revisión para CQ-4232374
...
* No se puede cambiar el tipo de recurso de imagen de medios mixtos del fragmento de contenido. NPR-21907: revisión para CQ-4233401
* Pantalla completa de RTE para el cuadro de diálogo de la IU táctil está ocultando los complementos de RTE. NPR-22034: revisión para CQ-4235457
...
* AEM Parsys apilado múltiple debido a consultas de larga ejecución (más de 6) hace que la aplicación sea lenta en el tiempo de ejecución de los mensajes de error en la aplicación de la. NPR-22134: revisión para CQ-4233904
* No se pueden cambiar los permisos en los nodos que tienen dos puntos en el nombre. NPR-22136: revisión para CQ-4236221
...
* Al mover sin permisos de replicación se inicia un flujo de trabajo de solicitud de activación si la página desplazada o sus referentes no se pueden activar. NPR-22467: revisión para CQ-4211765
...
* El filtro Omnisearch no muestra todas las acciones en la consola Sites. NPR-22804: revisión para CQ-4239007
...
* Incoherencia en el resaltado de extractos devuelto por la búsqueda de Lucene. NPR-22879: revisión para CQ-4238513
...
* El servlet del exportador Sling no exporta correctamente los caracteres japoneses. NPR-22153: revisión para CQ-4228920
* Interbloqueo del programador durante el inicio. NPR-22440: revisión para Sling-7004
* No se pueden sincronizar grupos entre dos instancias de publicación. NPR-21943: revisión para Granite-19564
* Problemas de rendimiento con el solucionador de sesión/recurso compartido org.apache.sling.i18n. NPR-23390: revisión para Sling-7543
...
* Al proporcionar un at.js o mbox.js personalizado, el script de inclusión se escribe en la página como texto en lugar de como etiquetas de HTML. NPR-22441: revisión para CQ-4203691
...
* Faltan recursos relacionados para la publicación de una página con contenido de destino. NPR-23064: revisión para CQ-4227119
...
* iParsys crea el cambio de posición después de desmarcar la casilla de verificación cancelar/deshabilitar herencia. NPR-21905: revisión para CQ-4230951
...
* Problemas con el selector de meses de Coral Calendar cuando se establece el intervalo de fechas &#39;min&#39; y &#39;max&#39;. NPR-22716: revisión para CUI-7187
...
* Habilite la creación de flujos de trabajo en el menú Fragmentos de experiencias. NPR-22347: revisión para CQ-4221661
...
* Se ha habilitado el identificador de PDF/usuario en el árbol de accesibilidad de Forms Designer.
...
* La contraseña se puede ver en texto sin formato en el campo de inspección de contraseña para las páginas de configuración de conectores de ECM. NPR-22508: revisión para CQ-4236065
...
* Independientemente del patrón de fecha seleccionado, el componente Selector de fecha no sigue el patrón al mostrar los mensajes de validación mínimos y máximos. NPR-22444: revisión para CQ-4236269
* El formato de fecha que se envía en la solicitud de envío debe coincidir con el patrón proporcionado en el componente selector de fechas. NPR-22384
...
* La codificación de entrada SOAP UTF-8 en el Forms adaptable devuelve errores con una página con errores. NPR-20105: revisión para CQ-4222669
* El componente Contenedor de AEM Forms no está disponible para editar una vez que se ha configurado un formulario incorrecto en la página de Sites. Revisión para CQ-4237456
* Las pruebas de desarrollo fallan cuando se ejecutan en servidores JEE. Revisión para CQ-4222082
* Pruebas AF que fallan en los servidores JEE debido a problemas de minificación en el marco de trabajo de Calvin. Revisión para CQ-4217220
...
* Las Forms que se modifiquen después de publicar la página no se volverán a publicar al publicar el sitio. NPR-23013: revisión para CQ-4236566

#### Integración de back-end {#backend-integration-1}

* La autenticación básica OOTB para los servicios SOAP no funciona para la autenticación básica en la integración de FDM. NPR-23238: revisión para CQ-4241308
...
* Error al iniciar el Planificador. NPR-22990
* Impida que los exploradores almacenen las credenciales introducidas en los formularios de HTML. NPR-21762: revisión para CQ-4206543
...
* Falta el identificador de PDF/UA al guardarse como PDF desde Designer. NPR-23137
* Los objetos de ruta, por ejemplo: bordes, subrayados e hipervínculos, no se etiquetan cuando se guardan como PDF desde Designer. NPR-23136
* El objeto de imagen no tiene cuadro delimitador cuando se guarda como PDF desde Designer. NPR-23134
...
* AEM Al abrir el paquete de datos XML con el Diseñador de Forms 6.3, se quita el formato XHTML del origen XML. NPR-21178: revisión para LC-3917046
...
* Excepción al intentar firmar o certificar digitalmente un documento de PDF a través de HSM. NPR-21154: revisión para CQ-4226978
...
AEM AEM El paquete de correcciones acumulativas 6.3.1.2 de es una actualización importante que incluye varias correcciones internas y de cliente desde que está disponible de forma general el paquete de servicio 1 (6.3.1.0) de 6.3 en octubre de 2017.

Los aspectos destacados del paquete de correcciones acumulativas de AEM son:

* Se ha modificado la interfaz de usuario para admitir la implementación de la funcionalidad CUG en AEM Assets.
* Se han corregido problemas de interfaz de usuario en la página de comprobación de licencias para mejorar la experiencia.
...
* Problemas de rendimiento al añadir usuarios a las colecciones. NPR-20699: revisión para CQ-4225733
...
* Se han resuelto varios problemas de administración de carpetas con las API de Assets. NPR-20569
* AEM Dynamic Media Classic (anterior Scene7 AEM) no puede sincronizar recursos desde el servidor de la cuando la ruta de destino en la configuración del servicio en la nube apunta a una subcarpeta en la ruta raíz. CQ-4228265
...
* Problemas de permisos de administración: no se pueden eliminar los miembros de un grupo de usuarios cerrado de las propiedades de página. NPR-20631
* El nombre del flujo de trabajo escrito debe estar disponible en el cuadro Notificación al publicar la página mediante Administrar publicación. NPR-20046: revisión para CQ-4221586
...
* Los fragmentos de experiencias no se muestran correctamente en Internet Explorer 11. NPR-20161: revisión para CQ-4223319
...
* El filtrado por etiquetas en páginas de colección en la consola de productos no funciona. NPR-20292: revisión para CQ-4224023
...
* Se muestra una paginación en blanco sin resultados para los usuarios anónimos de la comunidad. NPR-20136: revisión para CQ-4220738
* Configure el déclencheur de alertas cuando se detecte un UGC como correo no deseado o cuando un usuario publique en un sitio con moderación previa. NPR-20274: revisión para CQ-96850
...
* CommunityUserOperationExtension se ejecuta en una excepción de conversión de clases. NPR-20532: revisión para CQ-4224385
...
* La reactivación del segmento después de cualquier modificación falla y genera un error. NPR-20054: revisión para CQ-4218401
...
* requestAttributes no está sin establecer para las otras sentencias data-sly-resource. NPR-20639: Revisión para 4226206
...
* El acceso a diferentes proyectos desde la consola Proyectos tarda más en cargarse. NPR-20314
* AEM La instalación de la versión 6.3.0.1 elimina el almacén de claves del usuario dam-update-service. NPR-20018
...
* El campo de color se establece en &quot;siempre requerido&quot; a pesar de los atributos del cuadro de diálogo. NPR-19702
* La barra de desplazamiento no se muestra para el componente de varios campos en pantalla completa en Internet Explorer 11. NPR-20261: revisión para CQ-4219782
...
* No se puede establecer el valor del componente de conmutador entre true/false. NPR-21111
...
* Se ha reemplazado el procesamiento del tiempo del lado del servidor con el componente de tiempo de base.
* AEM Se han habilitado las etiquetas de publicación desde la consola de administración de etiquetas/etiquetado en Brand Portal desde el.
...
* Dynamic Viewer carga la representación de vídeo de menor calidad para probar la experiencia de reproducción de vídeo en el modo Autor. NPR-19004
...
* La visualización de caducidad de recursos está desactivada al cruzar zonas horarias positivas y negativas. NPR-20329: revisión para CQ-4219333
...
* AEM El campo Motor de segmentación del asistente de configuración se establece en ContextHub () en lugar de Adobe Target. NPR-19320: HF para CQ-4218465
* La sección Audiencia se daña al crear la experiencia. NPR-19110
...
* AEM Los valores desplegables de campos compuestos múltiples no se rellenan debido al código de cambio en Sidekick.js después de actualizar la instancia a la versión 6.1SP2-CFP3. NPR-19450: HF para CQ-4194771
* WCMode.EDIT no funciona para componentes de destino en modo de creación. NPR-19387
* Se ha publicado la API de JSON para consumir los fragmentos de contenido. NPR-19500
* La funcionalidad de negrita, cursiva y subrayado no funciona para los campos del editor de texto enriquecido en el cuadro de diálogo de autor. NPR-19670: NPR-19718: revisión para CQ-4219088
...
* No se puede generar la previsualización para proyectos de traducción. NPR-19289
...
* El correo se activa al publicar contenido con sitios moderados previamente. NPR-20008
...
* El botón Iniciar flujo de trabajo después de seleccionar un editor de variaciones de XF no tiene ningún efecto. NPR-19642: revisión para CQ-4207796
...
* La edición masiva de propiedades de página no se puede utilizar correctamente, ya que actualmente todas las pestañas y campos están disponibles para la edición masiva. NPR-19451
* Las propiedades de OOTB y las propiedades de alineación de imagen no aparecen al editar una imagen en la IU clásica. NPR-19488: HF para CQ-4217914

### Interfaz de usuario {#user-interface-4}

* Al utilizar el explorador Google Chrome, los usuarios no pueden cargar la lista completa de páginas/recursos mediante la vista de columna en la IU táctil. NPR-19566: revisión para CQ-4214248

### Brand Portal {#brand-portal-1}

* AEM No se pueden publicar recursos de la biblioteca de recursos con comentarios y anotaciones. NPR-19590: revisión para CQ-4218386
...
* Se ha corregido el campo &quot;habilitado&quot; en la IU de configuración de Brand Portal cloudservice. Revisión para CQ-4211101
* Error en la replicación del formulario de búsqueda. Revisión para CQ-4220080
...
* AEM El flujo de trabajo Enviar a JEE no devuelve el parámetro de salida del lado JEE a la dirección de correo electrónico y produce el error &quot;Ignorando porque el valor no es primitivo&quot;. NPR-20265
* El Forms adaptable no permite el PDF como archivo adjunto en Safari. NPR-19625
* RestoreGuideState sobrescribe el mapa customcontextproperty. CQ-4222877
...
* Los Forms adaptables basados en esquemas XSD envían valores XML erróneos para campos numéricos en instalaciones con configuraciones regionales que tienen un separador decimal distinto de &quot;&quot;. que genera errores. NPR-20444
...
* Los caracteres UTF-8 como salida del servicio SOAP no se copian correctamente en los campos de Forms adaptable para configuraciones regionales que no estén en inglés. NPR-20238: NPR-20103

#### Administración de correspondencia {#correspondence-management-1}

* Copiar y pegar contenido del archivo de Word pierde su color de contenido y su fuente en el Editor de texto. NPR-19521

#### Servicio del ensamblador {#assembler-services}

* Discrepancia entre los resultados de Acrobat AEM y los de la comprobación de la conformidad de un documento en formato PDFA-1b. NPR-19280

#### Flujo de trabajo {#workflow-2}

* Paso de firma de flujo de trabajo para permitir que las llamadas a http se conecten a través del proxy. NPR-20529

#### Formularios HTML5 {#html-forms-4}

* Se ha agregado compatibilidad con el evento previo al envío. NPR-20604
...
* El campo de texto multilínea (TextArea) no conserva el carácter de nueva línea ni el salto de texto después de enviar datos en HTML Workspace. NPR-20085
...
>[!NOTE]
>
>Instale el paquete incrustado de LiveCycle que aparece en [Versiones de AEM Forms](aem-forms-releases.md) para solucionar el problema.
...
* Cuando el servidor de AEM Forms que se ejecuta en una configuración de clúster en la aplicación JBoss se detiene, el servidor de aplicaciones se desconecta de la base de datos. Puede provocar problemas de corrupción de datos. NPR-19724

### Paquetes de funciones incluidos {#feature-packs-included-1}

* El campo de esquema de metadatos desplegable no se puede hacer obligatorio, ya que falta una validación de campo obligatoria para los recursos. NPR-17882: FP para CQ-4208373
...
* Auditabilidad del control de acceso del usuario
* Incluye la versión 1.6.2 del repositorio integrado (Apache Jackrabbit Oak)
* Problemas resueltos de resolución de recursos sin cerrar
...
* Edición mejorada de esquemas de metadatos en dispositivos híbridos
* Se han resuelto problemas de sincronización a nivel de componente en Live Copies
* Se ha mejorado la capacidad de uso de las páginas con contenido pesado en la vista de columnas
* Se ha mejorado el cumplimiento de la convención de nombres de páginas para páginas con títulos largos
* Se ha mejorado la experiencia de personalización de campañas en la IU táctil
* Correcciones de varios problemas de superposición de proyectos

### Plataforma {#platform-5}

* Revisión para Oak 1.6.2. NPR-16993
* Al abrir Omnisearch mediante un filtro, la ruta ya no se establece. NPR-17398: revisión para CQ-4204870
* AEM Requisito para auditar los cambios en los permisos de usuario en la. NPR-17061
* Conexiones persistentes a Cloud Services de DM que provocan excepciones de &quot;Demasiados archivos abiertos&quot;. CQ-4211407

### Assets {#assets-12}

* Problemas de uso con la configuración de los servicios de contenido inteligente mediante distintas opciones. NPR-18200: revisión para CQ-4201557
...
* El esquema de metadatos predeterminado provoca un error de validación de fragmentos de contenido. NPR-17769: revisión para CQ-4211111
...
* Solicitud para crear varios agentes de replicación para publicar recursos en Brand Portal. NPR-17189
* La tarea de revisión de los recursos de la carpeta en japonés no funciona. CQ-4204782
* La excepción de puntero nulo se produce cuando un recurso se mueve desde su página de propiedades. CQ-4204251
* AEM No se puede realizar un seguimiento de las referencias subsiguientes a un recurso en la página de propiedades si está vinculado a un documento de InDesign varias veces. CQ-4204186
...
* En las carpetas de recursos descargadas, si se produce un conflicto al extraer el contenido del archivo ZIP, la opción predeterminada aparece como Crear versión en lugar de Conservar ambos. CQ-97800
* Los usuarios con permisos de solo lectura en la aplicación no pueden obtener una vista previa del contenido desde la administración de contenido de AEM Mobile. NPR-17486: CFP para CQ-4209690
* El botón Crear catálogo no funciona en la vista de columnas de la consola Catálogo. CQ-4209952
...
* Problemas con la navegación por una página con mucho contenido en la vista de columna en la IU táctil. NPR-17799: revisión para CQ-4199611
...
* Problemas con la creación de páginas en la IU táctil en AEM 6.3 implementada en Jboss EAP 6.4. NPR-17589: revisión de CQ-4210137
* El proveedor de estado del flujo de trabajo hace que la instancia se bloquee cuando hay grupos anidados. NPR-17556: Solicitud CFP para CQ-4202056
...
* Problemas con la personalización de campañas y experiencias asociadas a través de la IU clásica. NPR-18127: revisión para CQ-4211559
...
* En un flujo de trabajo no transitorio, el historial de procesos y los cambios de metadatos realizados antes de un paso de proceso externo no persisten. NPR-17848: revisión para GRANITE-17757
...
* El error de fecha no analizable se produce al editar la tarea desde la bandeja de entrada. CQ-4208749
...
* Correcciones en los módulos de texto de Administración de correspondencia, vistas previas de cartas e inicio mediante programación de la interfaz de usuario de Crear Administración de correspondencia.
...
* Cuando el envío del HTML utilizó el método del POST, la interfaz de usuario de Crear correspondencia no se inicia. NPR-18202
...
* Cuando se añade contenido nuevo o se pulsa la tecla Intro, se añade una etiqueta div al módulo de texto. NPR-18240

**Assembler**

* Al validar un documento de PDF para la compatibilidad con PDF/A-1b, AEM Forms devuelve un error de validación: PDFA_CONTENT_003_DEVICE_DEPENDENT_COLOR_USED. El documento del PDF no devuelve el error cuando se valida con Comprobación previa de Adobe y software de terceros. NPR-18011
* Al validar documentos de PDF para la compatibilidad con PDF/A-1b, AEM Forms devuelve un error de validación: el campo de formulario tiene varias apariencias. Los documentos de PDF son compatibles con PDF/A-1b. NPR-18013

**Ver carpeta**

* Al seleccionar procesar archivos mediante un flujo de trabajo, mientras se crea una carpeta de inspección, la lista desplegable de selección del modelo de flujo de trabajo aparece truncada. NPR-17566
...
* La interfaz de usuario de AEM Forms enumera todos los recursos en el primer orden más antiguo. Los usuarios no pueden reordenar los recursos en el primer orden más reciente. NPR-18450
...
* El servicio PDF Generator no convierte imágenes de más de 100 MB en documentos de PDF. Ref# CQ-4208628
* Al utilizar el servicio PDF Generator con un OCR en japonés, se genera un PDF invertido. NPR-17602
...
* Microsoft Excel y Microsoft AEM PowerPoint tardan mucho más tiempo en abrir los documentos protegidos con la extensión de Document Security para Microsoft Office. CQ-4212358
...
El paquete de correcciones acumulativas 6.3.0.1 de AEM es una actualización importante que incluye varias correcciones internas y de cliente desde la publicación general de AEM 6.3 en abril de 2017. Los aspectos destacados del paquete de correcciones acumulativas de AEM son los siguientes:
...
>
>Puede acceder a la documentación de la opción Validar en [https://docs.adobe.com/content/docs/es-ES/aem/6-3/administer/content/package-manager.html#Package%20Validator](https://docs.adobe.com/content/docs/es-ES/aem/6-3/administer/content/package-manager.html#Package%20Validator)
...
* Para una implementación de clúster mediante RDBMK o MongoDB, el paquete CFP se puede instalar en cualquiera de las instancias de creación que usa el administrador de paquetes.
...
El registro de actividad (activity.log) se creará en crx -quickstart /logs.
...
* Utilice la [API de HTTP del Administrador de paquetes](https://helpx.adobe.com/es/experience-manager/6-3/sites/administering/using/package-manager.html) (asegúrese de utilizar `cmd=install&recursive=true`) para que se instalen los paquetes anidados.
...
1. Instale el paquete de complementos para Forms tal como se describe en la [instalación de paquetes de complementos para AEM Forms](https://helpx.adobe.com/es/experience-manager/6-3/forms/using/installing-configuring-aem-forms-osgi.html).
...
* Durante la instalación de AEM CFP 6.3.3.x pueden producirse los siguientes errores y advertencias, que se pueden ignorar sin problemas:
   ...
   * &#42;ADVERTIR&#42; [OsgiInstallerImpl] org.apache.jackrabbit.vault.packl.InstallHookProcessorImpl Hook /META-INF/vault/hooks/cloudservices-wfchangeinstallock-0.0.2-jar-with-dependencies.jar devuelve una excepción de tiempo de ejecución.
   * &#42;ERROR&#42; [OsgiInstallerImpl] com.adobe.cq.social.cq-social-jcr-provider [com.adobe.cq.social.provider.jcr.impl.SpiSocialJcrResourceProviderImpl(2174)] Tiempo de espera hasta que se complete el cambio regular sin registrar. CQ-4209974.
   ...
   * org.apache.sling.servlets.resolver.internal.SlingServletResolver: La llamada al controlador de errores produjo un error
   * org.apache.sling.servlets.resolver.internal.SlingServletResolver Error original nulo
   * org.apache.sling.engine.impl.DefaultErrorHandler Error del controlador de error:java.io.IOException
   * &#42;ERROR&#42; [FelixDispatchQueue] com.day.cq.dam.cq-dam-core FrameworkEvent ERROR (org.osgi.framework.ServiceException: El generador de servicios devolvió un valor nulo. (Componente: com.day.cq.dam.handler.standard.ps.PostScriptHandler))
...
>[!NOTE]
>
>Si tiene alguno de estos dos problemas, póngase en contacto con [AEM Atención al cliente](https://helpx.adobe.com/es/marketing-cloud/contact-support.html).
...
* Abra adobe-livecycle-cq-author.ear/cq.war/WEB-INF/web.xml para editar:
   ...
   * Después del cambio anterior, init-param debería tener un aspecto similar al siguiente:
...
**Documentación para agregar directivas ACL manualmente**

Para actualizar las directivas ACL, agregue los siguientes controles de acceso a través de CRX-DE:
...
La propiedad controla la profundidad máxima del subárbol de nodos bajo el nodo de página ` /  jcr   :content`, hasta la cual los nodos presentes en el repositorio se utilizarán para obtener las propiedades de página. Se omite cualquier nodo presente por debajo de la profundidad especificada en esta propiedad.

El valor predeterminado es 1. El valor se puede    sobrescribir superponiendo las constantes.js del archivo (`/libs/cq/ui/widgets/source/constants.js`) sin comentar la propiedad CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL y asignándole el valor necesario (la profundidad máxima bajo la propiedad de la página / jcr  :content hasta la cual se almacenan los datos de las propiedades de la página).

**Si el usuario necesita crear variantes de varias páginas para que el número de nodos bajo el nodo /  jcr   :content de la página sea superior a 1000, siga los pasos siguientes para realizar cambios en la configuración:**

* Configure la propiedad JSON Max resultados de Apache Sling
* Obtener servlet mediante `/system/console/  configMgr`
* Establezca su valor en un número >1000 (que es el valor predeterminado actual) de modo que este número sea mayor que el número total de nodos en el subárbol del nodo / jcr  :content hasta alcanzar la profundidad establecida anteriormente.

Esto permite que el servlet de sling GET pueda devolver todos los nodos necesarios.

## Uber Jar {#uber-jar}

Uber Jar para 6.3.3.8 está disponible en el [repositorio Maven público de Adobe](https://repo.adobe.com/nexus/content/groups/public/com/adobe/aem/uber-jar/6.3.3.8/).

Para usar Uber Jar en un proyecto de Maven, consulte el artículo [Cómo usar Uber Jar](https://helpx.adobe.com/es/experience-manager/6-3/sites/developing/using/ht-projects-maven.html) e incluya la siguiente dependencia en el elemento POM de su proyecto:
...
|----|-----|-----|-----|
| Integración de Assets y Adobe Creative Cloud | [AEM para uso compartido de la carpeta Creative Cloud](https://helpx.adobe.com/es/experience-manager/6-3/sites/administering/using/creative-cloud.html) se introdujo en AEM 6.2 como una forma de proporcionar a los usuarios creativos acceso a los recursos de AEM. Una nueva funcionalidad lanzada en la aplicación de Creative Cloud, Adobe AEM Asset Link, ofrece una experiencia de usuario mucho mejor y un acceso más potente a los recursos desde la directa desde Photoshop, InDesign y Illustrator.<br /> El Adobe no realizará más mejoras en la capacidad de uso compartido de carpetas. AEM Aunque la función está incluida en la, se recomienda encarecidamente a los clientes que utilicen la sustitución. | Vínculo de recursos de Adobe o aplicación de escritorio. Para obtener más información, consulte [AEM Integración de Creative Cloud de](https://helpx.adobe.com/es/experience-manager/6-3/assets/using/aem-cc-integration-best-practices.html) artículo. | AEM 6.3.3.x |
...
>[!MORELIKETHIS]
>
>* [Versiones y actualizaciones de AEM](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates.html?lang=es)
...
>* [Documentación de AEM 6.3](https://docs.adobe.com/content/docs/es/aem/6-3.html)
