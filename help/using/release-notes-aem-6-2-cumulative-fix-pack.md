...
description: Obtenga más información sobre las notas de la versión del Fix Pack acumulativo de AEM 6.2.
...
# Notas de la versión del Fix Pack acumulativo de AEM 6.2 {#release-notes-aem-cumulative-fix-pack}
...
El Fix Pack acumulativo 6.2 SP1-CFP20 de AEM es el último Fix Pack acumulativo para AEM 6.2 y es una actualización importante que incluye correcciones clave del cliente publicadas tras la disponibilidad general de [AEM 6.2 SP1](https://experienceleague.adobe.com/es_es/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).

>[!CAUTION]
>
>Aplicar CFP sin validar la compatibilidad entre los paquetes de funciones instalados puede provocar errores en el sistema o la pérdida de las configuraciones personalizadas, lo que podría necesitar la restauración desde la copia de seguridad para solucionarlo.

>[!NOTE]
>
>* Se incluye un nuevo paquete Sling `discovery- api` Johnzon 1.0.0 con el Fix Pack acumulativo de AEM 6.2 SP1-CFP10. Además, se ha añadido un usuario de servicio sling-discovery con privilegios de lectura y escritura para el nodo */var/discovery* en el repositorio de CRX.
>
...
>[!NOTE]
>
>Para los clientes de AEM Forms, es esencial instalar el paquete de complementos de AEM Forms después de instalar cualquier Service Pack, Fix Pack acumulativo o Feature Pack de AEM.
...
* Al obtener una vista previa de la letra como PDF, el campo de texto colocado en la página principal no respeta el valor introducido desde la pestaña de datos o según el vínculo de datos especificado. NPR-29239: revisión para CQ-4266856.
...
* No se puede cambiar el nombre de una carpeta DAM si falta el permiso Editar ACL. NPR-27555: revisión para CQ-104652
* La herramienta del editor de ajustes preestablecidos de imagen no responde en la versión 6.2.1 CFP 17 y posterior. NPR-28147: revisión para CQ-4261041
...
* La visualización del icono para interrumpir la herencia no es coherente. NPR-27671: revisión para CQ-4257779
...
* Se añadió la compatibilidad de redireccionamiento al componente de la página principal.
...
* La funcionalidad de descarga utiliza AEM Assets mediante un servlet de descarga de recursos, lo que permite a los usuarios anónimos descargar todos los recursos. NPR-27054, revisión para CQ-4254732
...
* Añadir la compatibilidad de redireccionamiento al componente de la página principal. NPR-26576: revisión para CQ-110529
...
* Copiar y pegar el texto insertando un &lt;br> no deseado en el fragmento de contenido. NPR-26660: revisión para CRTE-151
...
* Debido al carácter especial de guión largo, el editor tiene un problema para vaciar la caché. NPR-27199: revisión para CQ-4242790
...
* Si se utiliza la integración de Target OOTB, la segmentación de un componente procesa toda la página en lugar de un componente de destino vacío. NPR-25273: revisión para CQ-4248003
...
* El paquete cq-social-as-provider no se inicia con las versiones 3.x del controlador Mongo. NPR-26271: revisión para CQ-4252710
...
* La variable se rellena como Lista de tipo de datos, el subtipo es de cadena, pero se produce un error “no se puede forzar el objeto”. NPR-26194: revisión para CQ-4252287
...
* Revisiones proactivas a ataques de ejecución de scripts en sitios múltiples (XSS). NPR-21893: revisión para CQ-4223385
...
* Acción de publicar en la creación de saltos de página bloqueados. NPR-23970: revisión para CQ-4243203
...
* La creación del lanzamiento falla al iniciar el flujo de trabajo “Solicitar lanzamiento” en un proyecto. NPR-24255: revisión para CQ-4245030
...
* No se puede crear una versión en el primer intento. NPR-24774: revisión para CQ-4232176
...
* Se observa un alto uso de la CPU debido a demasiadas solicitudes en la funcionalidad de búsqueda de administración. NPR-23588: revisión para Granite-21286
...
* Cuando se obtiene una vista previa de una letra mediante la previsualización predeterminada, la asignación de fragmentos de diseño no se muestra, pero aparece correctamente cuando se hace clic en Previsualización. NPR-22993: revisión para CQ-4237745
...
* Correcciones de seguridad proactivas en la tabla Foundation para mantener la coherencia del diseño.
...
* Actualización del archivo adobe-reader-extension-dsc.jar más reciente para obtener correcciones en la extensión de Reader.
...
* Los datos EmbedXMP siempre se definen como “activos” para el proceso de generación de TIFF Pyramid. NPR-22776: revisión para CQ-4234498
...
* (Dynamic Media) Al seleccionar la casilla de verificación representaciones dinámicas, el archivo zip descargado genera la imagen TIFF original con un archivo de cero bytes. NPR-22410: revisión para CQ-4198471
...
* La variable de ruta no se codifica correctamente, lo que provoca la ejecución de scripts en sitios múltiples (XSS) no persistentes. NPR-22851
...
* Al guardar el segmento con un valor muy grande (10000000000), se establece el valor de ampliación en 0, lo que provoca un mensaje de error. NPR-22524: revisión para CQ-4237006
...
* El componente Captcha ya no se utiliza para mejorar la seguridad. Si utiliza el componente Captcha, aparece el mensaje “El componente Captcha está obsoleto y ya no debe utilizarse” después de instalar la versión 6.2 SP2-CFP15 o posterior. El componente de AEM se puede personalizar para incluir reCAPTCHA para mejorar la seguridad NPR-22151: revisión para CQ-4220052
...
* Cuando se obtiene una vista previa de una letra mediante la previsualización predeterminada, la asignación de fragmentos de diseño no se muestra, mientras que la misma aparece correctamente cuando se hace clic en el botón previsualización. NPR-23335: revisión para CQ-4237745
* Los datos de la letra correspondientes a los enlaces definidos en XDP no se rellenan usando la dirección URL de la letra directa. NPR-24145: revisión para CQ-4244290
...
* Los formularios incrustados en el sitio no se publican al volver a publicar la página del sitio. NPR-23014: revisión para CQ-4236566
...
* Vulnerabilidad de la ejecución de scripts en sitios múltiples (XSS) almacenada en la pestaña Notas del administrador de Forms. NPR-27157: revisión para CQ-4255556

#### Servicio de cifrado {#encryption-service}

* (OSGI) [JEE] Estado de firma incorrecto para el PDF con archivos adjuntos mientras se verifica con “Validar proceso del PDF”. NPR-23269, NPR-23737
...
* Los usuarios no pueden descargar recursos con renuncia de responsabilidad y nombres de archivo largos. NPR-22163: revisión para CQ-4235274
* El carácter de comillas simples evita la actualización de metadatos en la vista masiva y la IU se interrumpe al abrir las propiedades de un recurso mediante las acciones rápidas de la barra de herramientas. NPR-22317, NPR-22353: revisión para CQ-4236990, CQ-4236469
...
* (IE) Al crear cualquier carácter en el campo de texto enriquecido por primera vez, se añade un espacio final al carácter. NPR-22443: revisión para CQ-4235343
...
* (Digital Publishing Suite) Tanto la fecha de publicación como la fecha de portada son campos obligatorios en las publicaciones antes de que se cargaran en DPS. NPR-22484

### Comercio {#commerce}

* Varias ejecuciones de scripts en sitios múltiples (XSS) en el asistente de creación de catálogo de comercio. NPR-22344: revisión para CQ-4237017
...
* El despliegue de una estructura que se ha movido en un posible cliente principal a un `cq:moveTarget` incorrecto. NPR-22373: revisión para CQ-4232536
...
* El motor de destino (mbox.js, at.js) no utiliza URL estropeadas y utiliza las URL que contienen dos puntos que pueden dar error en determinadas implementaciones. NPR-22366: revisión para CQ-4237854
...
* El iniciador del flujo de trabajo no puede aceptar el valor TypeHint de String. NPR-22436: revisión para CQ-4237855
...
* (Espacio de trabajo HTML) Cuando un usuario reclama una tarea, el recuento de la cola se actualiza solo para ese usuario en particular. Es decir, a menos que se actualice la página. NPR-19778: revisión para CQ-4233665
...
* Se ha habilitado la configuración del campo Parámetro estático dentro de la configuración del componente objetivo al usar AT.js como biblioteca de cliente.
* Correcciones en la funcionalidad de mostrar u ocultar del componente desplegable.
...
* (Safari) Los usuarios no pueden ver todos los recursos en el panel del Buscador de recursos. NPR-21981: revisión para CQ-4213720
* La deformación de tiempo devuelve un error “RecursionTooDeepException” con una página ilegible y no se crea ninguna nueva versión aunque se cambie la fecha. NPR-21707: revisión para CQ-4199536
* Al cargar una página en el editor, WorkflowStatusProvider (pageinfo.json) se carga tres veces, lo que provoca que la instancia de AEM funcione con lentitud. NPR-21778: revisión para CQ-59232
...
* Los usuarios no pueden ver los valores de prueba Static Param en la llamada de mBox que se pueden ver cuando se realizan pruebas con AT.js como biblioteca de cliente en la configuración en la nube. NPR-21930: revisión para CQ-4234520
...
* Los usuarios no se sincronizan con la sincronización de usuarios mediante la distribución de Sling. NPR-21911: revisión para Granite-20404
...
* Falta la propiedad jcr para el id de usuario iniciador al crear un proyecto de traducción. NPR-21715: revisión para CQ-4230713
...
* Los valores especificados en la segunda opción de un campo con varios valores en el esquema de metadatos no se anexan a los valores especificados anteriormente en CRX-de. NPR-21220: revisión para CQ-4224526
...
* Al añadir un marco objetivo, el modo de segmentación no está disponible en la lista Seleccionar modo del editor. NPR-21047
...
* El proceso de restablecimiento de Live Copy se detiene después de eliminar la primera página local en LC. NPR-21276: revisión para CQ-4229743

### Plataforma {#platform-5}

* La biblioteca de etiquetas personalizadas que hace referencia a las etiquetas implementadas como un script no se encuentran tras actualizar a AEM 6.3. NPR-20149: revisión para Granite-18433
...
* El componente Seleccionar no elimina la propiedad objetivo durante el envío del formulario. NPR-21163: revisión para GRANITE-14125
* La ampliación HTTP.encodePathOfURI codifica dos veces el signo &#39;+&#39;. NPR-21417: revisión para GRANITE-16340

### Vulnerabilidad {#vulnerability-4}

* Ejecución de scripts en sitios múltiples (XSS) en el editor de metadatos de DAM. NPR-21434: revisión para CQ-83472
* Varios archivos SWF vulnerables a la ejecución de scripts en sitios múltiples (XSS). NPR-20612: revisión para CQ-4213297
...
* El botón de envío de la vista previa de la carta se desactiva después de instalar AEM 6.2 SP1-CFP9. NPR-21547
* Al seleccionar el tipo de vinculación de Recurso, la ventana Selector de recursos no se abre en el asistente Editar enlace de datos de letras. NPR-21164: revisión para CQ-4194567
...
* La actualización a la última actualización 131 de Java™ 8 genera una excepción: “el proveedor JsafeJCE está deshabilitado, ha fallado una comprobación de autointegridad requerida por FIPS 140”. NPR-21355

**Nota:** Este NPR requiere más ajustes. Consulte la [Última actualización de Java™ 8](#latest-java-update-throws-an-exception-npr).

* Los archivos jsafe.jars han sido actualizados a Cryptoj 6.1.3.1 en Núcleo, Cifrado, Firma y Document Security. NPR-21360, NPR-21361, NPR-21356, NPR-21358
...
* Los archivos jsafe.jars se han actualizado a Cryptoj 6.1.3.1 en RE. NPR-21357
...
* Los usuarios no pueden descargar recursos con renuncia de responsabilidad y nombres de archivo largos. NPR-20255: revisión para CQ-4222139
...
* El componente de imagen Sightly “vínculo a” genera el error 403 debido a la falta de la extensión .HTML. NPR-20823: revisión para CQ-4195909
...
* docConvertor falla al producir PDF/A con el error “El prefijo “stEvt” del elemento `stEvt:action` no está enlazado”. NPR-21032: revisión para CQ-4222540
* Se produce una excepción con el nombre `java.lang.IllegalArgumentException message:No enum constant com.adobe.internal.pdfm.docbuilder.signature.PathValidationFailureReason.SIGNED_IN_FUTURE` al invocar el servicio OMPFSubmission/PDFA/PDFtoPDFA. Esta excepción evita que el proceso de verificación de firma de breve se complete hasta que se reinicie el servidor. NPR-20792
...
* El proceso de preparación de datos se invoca varias veces cuando un formulario para dispositivos móviles se procesa con parámetros dataref. NPR-19801: revisión para CQ-4230427: CQ-4230400
...
* Los recursos en miniatura con caracteres especiales en el título muestran el objeto Java™ en el atributo alt de NPR-20347: revisión para CQ-4223620
...
* Los diccionarios de internacionalización no funcionan a menos que se reinicie el servidor en org.apache.sling.i18n 2.5.6. NPR-20525: revisión para Granite - 19490
* No se generan depósitos de hilos según el período del planificador con la configuración predeterminada del recolector de volcados de hilos (inicio AEM predeterminado). NPR-20288: revisión para GRANITE-19488/GRANITE-12741/CQ-90647.

### Sitios {#sites-9}

* Si el filtro de fecha modificada se cambia después de abrir la búsqueda guardada, no hay ningún efecto en los resultados. Y, los resultados mostrados son los mismos que el valor guardado anterior del filtro de fecha modificada. NPR-19739: revisión para CQ-4219425
...
* Cuando se accede mediante una sesión anónima, ClientLibraryProxyServlet no redirige mediante solicitudes proxy a las bibliotecas de cliente en la instancia publicada y genera un error HTTP 404 no encontrado. NPR-20195: revisión para Granite-14409

### Integración {#integration-10}

* Si se selecciona el motor de destino como Adobe Target, se evita que el componente se cargue y genera un error en el registro del servidor. NPR-20058: revisión para CQ-88071, CQ-109698, CQ-4201600

### Comercio {#commerce-1}

* No aparece ningún mensaje emergente de confirmación o redireccionamiento al crear productos de la misma página. NPR-20257: revisión para CQ-4223414
...
* La propiedad ImageMap sigue existiendo incluso después de eliminar las imágenes del componente de editor in-situ de varias imágenes. NPR-20142: revisión para CQ-4222982
...
* La secuencia de comandos valueCommit se ejecuta dos veces para DropDownList cuando se cambia a través de la IU. NPR-19989: revisión para CQ-110212
...
* El formulario rellenado previamente no funciona cuando el formulario para dispositivos móviles está configurado para visualizarse como un PDF. NPR-20566

**Rights Management**

* El cuadro de diálogo de selección de certificados de autenticación mutua o CAC debe mostrar los certificados con uso mejorado de clave (EKU) como inicio de sesión con autenticación de cliente o tarjeta inteligente. NPR-20708
...
Esta experiencia proporciona un nuevo Dynamic Media que se conecta a la nueva infraestructura de publicación de Dynamic Media para una replicación más rápida y escalable.
...
* AEM muestra un comportamiento inesperado cuando un usuario intenta publicar una versión `/unpublish/create` o crear una versión de una página bloqueada por otro usuario. NPR-19249; revisión para CQ-4215298 y CQ-4203856
* Se elimina la página secundaria al promocionar el lanzamiento anidado manualmente. NPR-19704
* Problemas de persistencia cuando los almacenes de ContextHub sobrescriben la capa de persistencia predeterminada durante la inicialización. NPR-19979: revisión para CQ-4218399
* Los fragmentos de contenido se superponen con otros botones. NPR-19980: revisión para CQ-4221519
* La página del sitio no se muestra cuando se ve mientras se usa un alias en SiteAdmin. NPR-20053: revisión para CQ-4221478
...
* Habilitación de las etiquetas de publicación de AEM a Brand Portal desde la consola de administrador de etiquetas/etiquetado. NPR-20271
...
* Formularios adaptables mejorados para adjuntar archivos PDF como archivos adjuntos en Safari. Para admitir la misma capacidad en los formularios existentes, cambie la configuración en el widget del archivo adjunto y en la actualización “Tipos de archivos admitidos”, actualice el valor application/pdf en lugar de .PDF. NPR-19623
...
* Problemas con la presencia de varios componentes de RTE en un cuadro de diálogo. NPR-19311: NPR-19587
* La depuración automática de versiones iniciales de Vanilla AEM 6.2 solo funciona después de que se inicialice VersionManagerImpl. NPR-19315: revisión para CQ-4217175
* La instancia de flujo de trabajo se queda atascada en el paso del flujo de trabajo “Exportación de Salesforce.com”. NPR-19222: revisión para CQ-4212976
...
* El servicio Plantilla de correo electrónico de Adobe añade etiquetas a las plantillas de usuario personalizadas. NPR-19190: revisión para CQ-4217113
...
* Renderizado lento de las miniaturas debido a artículos de tamaño completo en la consola de artículos de AEM. NPR-19086: CQ-4217148
...
* El cuadro de diálogo de segmentación no se muestra en el modo de segmentación cuando se edita un módulo de destino y se guarda más de una vez. NPR-19144: revisión para CQ-4216708

### Flujo de trabajo {#workflow-2}

* Los usuarios no pueden filtrar las notificaciones en la Bandeja de entrada por usuario o grupo en la Bandeja de entrada IU clásica. NPR-19122: revisión para CQ-4215374
...
* El contenido sin salto de línea en el Editor de texto no se puede ajustar. NPR-19481
...
>* Los resultados de la búsqueda no se resaltan cuando parte de la palabra buscada tiene un estilo diferente o contiene caracteres especiales como &quot;, &#39; o \.
>* La búsqueda no funciona para el contenido dinámico (como los valores de elementos del diccionario de datos o los valores de variables) dentro del fragmento de documento.
...
* La pantalla de administrador de Microsoft® Windows muestra la versión número 6.0 después de instalar CFP6. Revisión para CQ-4217573
...
* Cambio de comportamiento en la visualización de títulos en la tarjeta de imagen para una imagen que tiene la propiedad dc: title establecida en String (multicampo).
* Correcciones en el problema de rendimiento con las interfaces de Servicios de nube de Dynamic Media, IU táctil e IU de seguridad.
...
* Los botones de acción Publicar están visibles incluso cuando el usuario no tiene acceso de replicación. NPR-17353, revisión para CQ-4209269
* Tanto Site admin como Misc admin no funcionan cuando la minimización está habilitada mediante min:gcc;obfuscate=true. NPR-18593, revisión para CQ-4209220
...
* La aplicación de permisos para macros DM rompe la vista para el usuario admin. NPR-18544, revisión para CQ-4211729
...
* El programa de instalación de JCR no actualiza la versión del paquete después de la instalación del paquete. NPR-18728; revisión para NPR-15135
* La replicación sin binarios (BLR) falla con entornos de creación y publicación de binarios. NPR-18704
* Solicitud de resolución del puente HTTP de Apache Felix en un entorno de AEM. NPR-18297
...
* Habilitación de las etiquetas de publicación de AEM a Brand Portal desde la consola de administrador de etiquetas/etiquetado. CQ-4212165
...
* Si existe una declaración XML en un archivo XML de datos y la representación de la letra se inicia mediante una petición POST, la letra correspondiente no muestra los datos. NPR-18870
...
>
>* Copiar y pegar desde Word/Web en el editor de texto de CM muestra el contenido de salto de línea. NPR-19530
>* El contenido sin salto de línea en el Editor de texto de CM no se puede ajustar. NPR-19449
>

#### Formularios adaptables {#adaptive-forms-9}

* Al añadir un nuevo panel para paneles repetitivos, se elimina el valor del campo desplegable del panel anterior. NPR-18772
...
* La secuencia de comandos para cambiar el título del botón en el evento de inicialización del panel raíz de guía no funciona. NPR-18476
...
* Administración eficaz de los componentes ocultos en el modo de diseño de la tableta.
...
* La acción de compra o los flujos de trabajo no están disponibles en la consola Línea de tiempo una vez aproximadamente. Se añaden 80 fragmentos en la carpeta Recursos. NPR-18257; revisión para CQ-4211214 y NPR-18251; revisión para CQ-4211216.
...
* La personalización solo funciona para la primera IP cuando la regla de segmentación está establecida para un rango de IP. NPR-18121, revisión para CQ-83767
...
* No se puede seleccionar una imagen después de quitar otra imagen de la página. NPR-17711, revisión para CQ-4201323
...
* El recorrido de la consulta se produce cuando el modelo de catálogo no tiene ninguna condición definida para una sección. NPR-18229, revisión para CQ-4211924
...
* Solicite utilizar una versión actualizada de servlet org.apache.sling.servlets.post (2.3.22) en la API de Apache Sling para evitar una vulnerabilidad XSS. NPR-18963

### Traducción {#translation-6}

* El envío de la página del recurso no debe ser necesario de nuevo para un proyecto de traducción hasta que se complete el proyecto. NPR-18249, revisión para CQ-4209908
...
* Cuando se procesa un componente de imagen HTL, se cambia el nombre del archivo en la dirección URL, lo que provoca una URL rota. NPR-17908, revisión para CQ-4211587
* No se puede salir de las propiedades de página después de realizar cambios. NPR-17832, revisión para CQ-96110
...
* El diccionario de datos se lee repetidamente durante la representación de la carta. NPR-18482, revisión para CQ-4210805
* Se ha añadido JavaDocs para la clase com.adobe.livecycle.content. NPR-18467
* Al crear una carta, no se guarda su descripción. NPR-18039
...
* Se ven errores inesperados en los registros en la instalación de un paquete de complemento. NPR-18295
...
* No se puede firmar un documento PDF con un Módulo de seguridad de hardware remoto. NPR-18017
...
* Se ha mejorado la capacidad de respuesta de la barra de acciones al visualizar acciones de la barra de herramientas y componentes de Coral.
...
* Los usuarios con permisos de “Solo lectura” en la aplicación AEM Mobile no pueden previsualizar el contenido de la página Administración de contenido de AEM Mobile. NPR-17390, revisión para CQ-4209690
...
* La IU de Administración de correspondencia no se inicia cuando la acción de envío HTML utiliza el método POST. NPR-17595

**Administrador de Forms**

* Con un formulario adaptable configurado para AB Testing, al hacer clic en Iniciar AB Testing no se inicia la prueba y se genera un error de consola del explorador. NPR-17838
...
* El uso del servicio de salida de AEM Forms 6.2 para combinar un formulario específico con un XML de datos tarda 20 veces más tiempo. Se corrigió en entornos de Windows y Linux®. NPR-17501
...
* Durante la edición de fragmentos en el editor de texto durante una previsualización de carta, el texto procesado debe mostrarse para su edición en lugar de las condiciones en línea utilizadas en los fragmentos. NPR-15748, NPR-17504
...
* Añadir o personalizar columnas a los resultados de **OmniSearch de Assets** superponiendo en `/apps` no funciona. NPR-16737: revisión para CQ-4206785
...
* El número de resultados mostrados en la pantalla de Administración de usuarios es diferente en la vista de tarjeta y lista. NPR-16241; revisión para GRANITE-16914
...
* La creación de un PDF dentro de otro PDF en la IU de Assets no muestra los PDF creados en la IU de DAM, Sin embargo, los PDF son visibles en el repositorio de CRX. NPR-16833: revisión para CQ-4206501
...
La Consola de flujos de trabajo de proyectos muestra una NullPointerException en la página cuando se depuran flujos de trabajo. NPR-17017: revisión para CQ-4194269

### Sitios {#sites-15}

* Los archivos de `ContextHub` no se minimizan en la instancia de autor. NPR-17022: revisión para CQ-79456
* La promoción del lanzamiento de WCM-Launches tarda mucho tiempo en promocionar un lanzamiento que consta de un gran árbol de una página. NPR-16480: revisión para CQ-82731
...
* En WCM Launches, las páginas asociadas con un lanzamiento no pueden cancelar la publicación cuando la acción se inicia desde las propiedades de la página en la IU táctil. NPR-16560: revisión para CQ-4204681
...
* En el importador de WCM-Design, abrir una página del importador de pruebas e intentar cargar un archivo zip provoca problemas si se ha cargado y eliminado anteriormente. NPR-16486: revisión para CQ-90962
* Navegar al panel **[!UICONTROL Navegación global]** mediante los navegadores Firefox o Google Chrome ofrece una experiencia de usuario diferente. El explorador Firefox muestra el menú **[!UICONTROL Herramientas]**. El explorador Chrome de Google muestra el menú **[!UICONTROL Navegación]**. NPR-16770, revisión para CQ-4200456

### Campaign {#campaign}

* Al probar plantillas de campaña de AEM y modificar direcciones semilla para incluir “Datos adicionales”, la lista desplegable de Adobe Campaign desaparece en el Context Hub de la IU táctil. NPR-16771: revisión para CQ-105748

### Mobile On-Demand {#mobile-on-demand-3}

* Al comprobar previamente una publicación desde el entorno de autor de AEM, una acción de comprobación preliminar que dure más de cinco segundos provoca un pico inusual en el panel de Splunk de integración de AEMM - AEM PECS con un número elevado de solicitudes de estado por segundo. NPR-16908: revisión para CQ-4207055
...
* En el editor de formularios adaptables, el comentario Configuración de destinatario en head.jsp debe reemplazarse por la nueva instrucción de Context Hub. NPR-17173
* En el editor de reglas de formularios adaptables, **[!UICONTROL Elegir un elemento]** muestra el evento como “nulo”. NPR-17139
* El formulario enviado se vuelve a enviar al navegar hacia delante mediante la flecha de avance (>). NPR-17080
* Al enviar un formulario adaptable mediante AJAX, la función de devolución de llamada “error” nunca se invoca si había un error. NPR-17034
...
* Compatibilidad con expresión regular de tipo `dataRef=C:/Users/`en la **[!UICONTROL Configuración del servicio de relleno previo predeterminada]**. Este tipo es para el relleno previo de datos para formularios adaptables. NPR-16425
...
* `Upload Comments` para archivos adjuntos que no se muestran en la interfaz de usuario para instancias enviadas, aunque estén presentes en el XML almacenado en el repositorio CRX. NPR-17075
...
* El Servicio de salida no puede procesar un PDF que se ha modificado para incluir el campo adicional &#39;milli-sec&#39; en los metadatos `date-time format`. NPR-16838
...
* Correcciones para usar públicos de sincronización de destinatarios, publicar campañas y seleccionar la Métrica de objetivo en la IU táctil
* Resuelve problemas de sincronización para trabajos de traducción
* Proporciona seguridad mejorada para el servicio de cumplimentación previa de Forms
* Mejoras en el componente de borrador y envíos del portal de Forms y en el Servicio de formularios de códigos de barras
...
* Opción de validación en el administrador de paquetes para detectar cualquier conflicto entre el archivo superpuesto (archivo JSP o JavaScript) en `/apps` y el que contiene una revisión en `/libs`. A continuación, la superposición afectada se puede volver a basar para incluir los cambios del archivo en `/libs`. NPR-16216: revisión para CQ-81729
...
* Después de actualizar de AEM 6.0 a AEM 6.2, la IU clásica muestra un rendimiento lento al buscar etiquetas debido a numerosas consultas. Para resolver el problema, se pueden seguir los pasos mencionados en [Deshabilitar el estado de replicación en la consola de etiquetado (IU clásica)](#disable-replication-status-in-tagging-console-classic-ui-npr). NPR-15842: revisión para CQ-4201748.

* Al crear una página en la IU táctil, la Comprobación de entrada del campo “nombre” no comprueba el carácter especial “Apóstrofo” (igual que en la IU clásica). Por lo tanto, la página no se puede mover. NPR-16404: revisión para CQ-4205321.
...
* Al desplazarse por las Pestañas de páginas del panel Buscador de contenido, algunos conjuntos de páginas se muestran indefinidamente en la IU clásica, mientras que la IU táctil muestra un conjunto limitado de pocas páginas no repetidas. NPR-16271: revisión para CQ-4202371
...
* El usuario no puede cargar recursos ni crear carpetas mientras navega por los recursos debido a un comportamiento inconsistente que se muestra al utilizar el botón “Crear”. NPR-16410: revisión para CQ-4204793
...
* Al habilitar la autenticación proxy con Autenticación implícita, el componente Búsqueda de AEM genera una excepción ConcurrentModificationException. NPR-15309: revisión para CQ-4199191
* Al crear una Actividad de prueba A/B de Target en AEM, el público no se sincroniza con Target y muestra “sin público”. NPR-16229: revisión para CQ-4204210
...
* Al usar la segmentación, la publicación de la campaña no publica automáticamente todo el árbol, incluidas la marca y la parte primaria. NPR-15855: revisión para CQ-94630

### Traducción {#translation-8}

* El Trabajo de traducción de sincronización no se activa automáticamente y el sondeo de AEM ahora se produce sin bloquear los proyectos de traducción. NPR-15163: revisión para CQ-90856
...
* Al guardar un Formulario adaptable con un archivo adjunto, la ruta completa del archivo adjunto se añade en la etiqueta `fileAttachment` del XML generado, en lugar del nombre del archivo adjunto. NPR-16529
...
* En el caso de los widgets de archivos adjuntos que permiten varios archivos adjuntos, al enviar una nueva instancia de formulario con un archivo adjunto en un widget que ya tiene un archivo adjunto anterior, se muestra un código de error. Este error se produce al abrir el archivo adjunto añadido en lugar del contenido real. NPR-16258
* La seguridad del servicio de formularios permite rellenar automáticamente el servicio de acceso no autorizado a través de protocolos como `file://`, `http://` y `ftp://`. Consulte [Configuración del servicio de relleno previo mediante el administrador de configuración](https://experienceleague.adobe.com/es_es/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions). NPR-15414

* Solicitud para procesar el formulario adaptable en formato PDF en lugar de HTML en el paso de verificación y anexar todos los archivos adjuntos al PDF, para que la copia impresa muestre el formulario completo. NPR-9011

**Administración de correspondencia**

* Mientras se trabaja con fragmento de texto en una carta en modo Vista previa o Edición, si el texto se convierte en lista, se interrumpe toda la funcionalidad de la carta y se genera un error de JavaScript. Además, se genera un error de JavaScript. NPR-16460
...
* Si aparece un campo flotante como un campo de entrada de un fragmento XDP, el campo no se expande en la vista previa de la carta mientras se utiliza el explorador Internet Explorer. NPR-16367
...
* Al trabajar en un fragmento de documento de texto con el explorador Internet Explorer, ak ntentanr proporcionar sangría al texto se produce un error, ya que el cursor no permite la sangría del texto. NPR-16128
...
* Al crear o editar fragmentos de documento de texto con el explorador Google Chrome, la lista desplegable de selección de color emerge automáticamente y no se puede eliminar. El usuario debe seleccionar lista como tipo de entrada de datos para poder editar el fragmento. NPR-16067
...
* Proteger el servicio de relleno previo de formularios del acceso no autorizado a través de protocolos como “file://”, “http://” y “ftp://”. Para obtener más información, consulte [Configuración del servicio de relleno previo mediante el Administrador de configuración](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions). NPR-15434
...
* Al eliminar cualquier recurso en la administración de correspondencia, se debe registrar un mensaje de advertencia en el archivo de registro error. NPR-13225
* Mejorar la funcionalidad de búsqueda durante la visualización previa de una carta en la administración de correspondencia, para habilitar la búsqueda de texto en el contenido del fragmento de texto, en lugar de buscar únicamente en el título o la etiqueta de la carta. NPR-16103
...
Los aspectos destacados del Fix Pack acumulativo 2 son:

* Correcciones de estabilidad y mejoras de rendimiento en la plataforma AEM, incluido el marco de trabajo y las operaciones de Sling
...
* El valor de las etiquetas con la propiedad `tagBasePath` no se recupera en el cuadro de diálogo de la IU clásica al desplazarse a un componente de página.  NPR-15543: revisión para CQ-4199950
...
* Un usuario selecciona un nodo en CRXDE (por ejemplo, `/content/dam/`). A continuación, hace clic en la pestaña &#39;Control de acceso&#39;, asegurándose de que existe una Lista de control de acceso. Ahora, al arrastrar y soltar algunos elementos, se mueven los elementos, no el seleccionado. NPR-15696 revisión para GRANITE-16300
...
* El estado de modificación de la Bandeja de entrada del flujo de trabajo en el WCM principal no cambia si el usuario modifica una página después de activar un flujo de trabajo. NPR-15684: revisión para CQ-4196974
* El complemento anclaje del Editor de texto enriquecido para la IU táctil genera un HTML5 no compatible cuando el usuario hace clic en el icono de anclaje y añade un nombre. Debe agregar un atributo “id” en lugar del atributo “name” en la etiqueta HTML5 para el elemento de anclaje. NPR-15650: revisión para CQ-89782
...
* En la IU del editor de fragmentos de contenido, en caso de que se creen muchas variaciones de un fragmento de contenido, el panel lateral no muestra la barra de desplazamiento para navegar por todas las variaciones. NPR-15445: revisión para CQ-4199444
...
* Falta el botón Borrar en la pestaña de miniaturas de propiedades de la página del componente Sitios. NPR-15143 revisión para CQ-4196997
* Para un sitio que utiliza Live Copies, al seleccionar la casilla de verificación “Live Copy” en el panel Columnas de Admin Console del sitio no se muestra el estado de Live Copy correctamente y solo se muestra el código HTML. NPR-15108: revisión para CQ-97086
...
* Después de buscar un recurso en OmniSearch, al seleccionar un recurso e intentar editar las propiedades al hacer clic en **Propiedades de vista** y luego en el botón **Guardar**, se redirige a los usuarios a una página en blanco. NPR-15900: revisión para CQ-4202372
...
* Al publicar un recurso desde la Vista de tarjetas, la Tarjeta no se actualiza para reflejar un estado publicado. En su lugar, se actualiza la página. NPR-15826: revisión para CQ-102732
* Revisión acumulativa que contiene Revisiones de recursos. NPR-15225
* Si se incluye el carácter de símbolo de ampersand (“&amp;”) en el nombre de una carpeta de recursos, el nombre de la carpeta no se muestra correctamente al desplazarse al recurso. NPR-15775: revisión para CQ-4201735
...
* El acceso a la IU de “Mis vínculos compartidos” mediante el explorador Firefox no muestra los elementos o usuarios compartidos y la pantalla no se puede utilizar. NPR-15539: revisión para CQ-4200992
* Al utilizar el administrador de recursos digitales, si una página está asociada a un conjunto de imágenes, al mover las imágenes a una nueva carpeta se interrumpe la asociación de páginas. Como tal, la página asociada omite algunas de las imágenes. NPR-15538: revisión para CQ-111479
...
* Cuando se utiliza la consola del Buscador de recursos en la vista de columna, al intentar crear una carpeta se produce un error una vez, aunque se logra al reintentarlo. NPR-15370: revisión para CQ-4199448
* Si un recurso o una carpeta seleccionado en la IU de DAM tiene una coma en el nombre, la pestaña Referencias no está disponible y muestra el mensaje “La lista de referencias no está disponible para varias selecciones”. NPR-15362: revisión para CQ-4199721
...
* En la **[!UICONTROL IU táctil]**, la fecha guardada no se muestra para los campos de fecha que no tienen type=&#39;datetime&#39; mientras se usa el explorador Internet Chrome versión 56.0.2924.87. NPR-15383: revisión para GRANITE-16481
...
* OmniSearch no permite que los clientes que utilizan `/apps` añadan una fuente de datos de columna, ya que supone que la configuración de ubicación se encuentra en */libs/granite/omnisearch/content/metadata/*. NPR-13188 revisión para GRANITE-16479
...
* La ejecución del flujo de trabajo de Scene7 da como resultado archivos abiertos que no se cierran. Solicite mejorar el servicio AEM-S7 para que mantenga y reutilice una única instancia de HttpClient con la configuración de agrupación compartida. NPR-15357: revisión para CQ-109958
...
* Mientras se utilizan los componentes de integración de Campaign, Segmentación y `MAC`, Test y Target, la anulación de la publicación de actividades no actualiza el estado de actividad en la IU primaria. NPR-15401: revisión para CQ-4199839
...
* El envolvente `afData/afBoundData` está presente en el XML enviado, aunque incluso el XML rellenado previamente no tiene un envolvente `afData/afBoundData` en los formularios adaptables basados en esquema XML. NPR-15541
...
* El servicio de relleno previo no funciona para Formularios de XDP. NPR-15466
...
* Al utilizar el servicio de salida para combinar un formulario específico con un dataxml, el tiempo de respuesta es 20 veces mayor que el tiempo que tarda el servidor ES4 SP1. NPR-15283
...
* Solicite la actualización a Jackrabbit File Vault a 3.1.30 para resolver varios problemas. NPR-13454
...
* Las listas desplegables de Target no funcionan en el cuadro de diálogo de la IU táctil cuando se crean varios campos de texto enriquecido, como, por ejemplo, vínculos. NPR-13911
...
* La información se carga lentamente en la pestaña Usos activos del Editor de plantillas. NPR-14880: revisión para CQ-83417
...
* Cuando se envía el cuadro de diálogo Propiedades de página desde el Editor, MSM Post Processor escribe algunos parámetros de la solicitud en lugar del parámetro `msm:writeLiveCopyConfig`. NPR-14434
* Varios problemas relacionados con el componente de Despliegue, Live Copies y otros aspectos de MSM. NPR-12235

### Recursos {#assets-18}

* El flujo de trabajo Unpack no puede gestionar imágenes con caracteres especiales en el nombre del archivo de imagen. NPR-15227: revisión para CQ-103887
* Los recursos que tienen la expresión Repetir con Condición no se muestran correctamente. Cuando el usuario previsualiza la plantilla de carta `*CDN3835RLCEN*`, no se muestra ningún recurso ubicado en el Área del cuerpo de destino. En el caso de los widgets de archivos adjuntos que permiten varios archivos adjuntos, al enviar una nueva instancia de formulario con un archivo adjunto en un widget que ya tiene un archivo adjunto anterior, se muestra un código de error. NPR-14844
...
* Al intentar editar los perfiles de vídeo de un vídeo en el Editor de canales, el cuadro de diálogo de diseño no se abre y genera una excepción de puntero nulo en el registro de errores. NPR-14144: revisión para CQ-81101
* La propiedad de marca de tiempo “Creado” generada por el sistema que se muestra en la página de propiedades de un recurso no es correcta. NPR-13992: revisión para CQ-95029
...
* Si se seleccionan varios recursos, se produce un error con el Código de respuesta 414 (URI de solicitud demasiado largo) si el URI de recurso o de la carpeta es demasiado largo. NPR-13516: revisión para CQ-76076
...
* Los usuarios de AMS acceden a la instancia de autor de AEM Assets desde redes distintas que no pueden cargar recursos mediante la carga de fragmentos sin privilegios de eliminación en carpetas. NPR-12768: revisión para CQ-82715
...
* Desde la actualización de `Jquery`, los usuarios no pueden seleccionar una colección de recursos y confirmar la selección en el panel Asociar contenido de un fragmento de contenido. NPR-14847: Backport para CQ-4194209
...
* El vínculo Carga útil de la pestaña Archivo de la consola de flujo de trabajo devuelve un error con el código de respuesta “404” al hacer clic en él. NPR-14993: revisión para CQ-4194977
...
* Si establece un valor predeterminado para un botón de opción y selecciona un valor diferente al guardar o enviar, el relleno previo no muestra el valor seleccionado. NPR-15304
...
* Cuando se utiliza el modo de edición para formularios adaptables, se detectan errores de JavaScript como “El controlador del componente no es válido” y “Guidelib no está definida”. NPR-15112
...
* Al navegar en el directorio *FormsandDocuments*, el botón Pegar no se muestra cuando el usuario copia cualquier recurso y luego navega a una carpeta diferente. CQ-111327
...
* Forms Designer se bloquea cuando el usuario ejecuta la utilidad de revisión de ortografía con la configuración regional del formulario seleccionada como francés (Canadá). NPR-13740
...
>[!NOTE]
>
>Para los clientes de AEM Forms, es esencial instalar el paquete de complementos de AEM Forms después de instalar cualquier Service Pack, Service Pack acumulativo o Feature Pack de AEM.
...
* Para una implementación de clúster mediante RDBMK o MongoDB, el paquete CFP se puede instalar en cualquiera de las instancias de Autor que usan el Administrador de paquetes.

* Antes de instalar el Fix Pack acumulativo, asegúrese de tomar una instantánea o realizar una copia de seguridad de su instancia de AEM.
...
1. Todos los paquetes OSGI son ACTIVE o FRAGMENT en la consola OSGI (utilice la consola web:/system/console/bundles).
...
* Superponga *“/libs/cq/tagging/widgets/source/widgets/admin/TagAdmin.js”* en *`/apps`*
...
NPR-21355 está incluido en CFP12.1. Si va a instalar CFP12.1 o posterior, realice el siguiente procedimiento para configurar NPR-21355 en el servidor de aplicaciones JBoss®. Si va a instalar CFP12.1 en el servidor de AEM Forms que se ejecuta en los servidores de aplicaciones de Oracle WebLogic o IBM® WebSphere, no se requiere ninguna configuración adicional:

1. Realice una copia de seguridad, elimine y cree el archivo module.xml. La ubicación predeterminada del archivo es [AEM_Forms_Installation_directory]/jboss/modules/system/layers/base/com/adobe/livecycle/main/
...
1. Cree una copia de seguridad de los archivos `jsafeFIPS.jar`, `jsafeJCEFIPS.jar` y `certjFIPS.jar` en [AEM_Forms_Installation_directory]`/jboss/modules/system/layers/base/com/adobe/livecycle/main/` y elimine los archivos del directorio mencionado anteriormente.
...
El recuento de la Cola compartida no se actualiza de forma predeterminada para otros usuarios cuando un usuario reclama una tarea. Para ello, Adobe ha introducido una nueva propiedad. Siga los pasos a continuación para configurar esta propiedad en la instancia de AEM:

1. Vaya a la IU de Administración -> Services -> Workspace -> Global administration.
1. Exporte la configuración Global.
1. En el archivo XML descargado, añada la etiqueta `<client_tasksPollingInterval>10</client_tasksPollingInterval>`. Diez es el valor de ejemplo en segundos. Puede modificarlo en consecuencia.
1. Guarde el archivo.
...
* Se recibe HTTP 500 Error interno del servidor al abrir la página de detalles del componente de la consola web.
* Se producen errores como **crear instancia de componente** y **la fábrica de servicio devolvió un valor nulo** debido al reinicio del repositorio:
...
* Al instalar CFPx en AEM 6.2 SP1 que incluye el Feature Pack de Etiquetas inteligentes, el paso del flujo de trabajo añadido anteriormente para Recursos de etiquetas inteligentes se elimina del flujo de trabajo de Recurso de actualización de DAM.

## Uber Jar {#uber-jar}

Uber Jar para 6.2 SP1-CFP20 está disponible en el repositorio de Maven público de Adobe.
